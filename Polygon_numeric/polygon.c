...
typedef enum { INCREASING, DECREASING } growing_state;

double calculateAndPrintSideError(point* vertice);
...
int main(int argc, char* argv[])
{
	...
	while (i < STEPS) {
		...
		int current_size =
			(int)hypot((vertice[0].x - CENTER_X), (vertice[0].y - CENTER_Y));
		...
		i++;
		if (i%10000==0)
			printf("%+.5e\t%+.5e\n", ini_val, calculateAndPrintSideError(vertice));
	}
...
point rotateVertice(point vertice)
{
	...
	vertice.x = CENTER_X + ((old_value.x - CENTER_X) * cos(ROT_STEP)) -
				((old_value.y - CENTER_Y) * sin(ROT_STEP));
	vertice.y = CENTER_Y + ((old_value.x - CENTER_X) * sin(ROT_STEP)) +
				((old_value.y - CENTER_Y) * cos(ROT_STEP));
	return vertice;
}
void initializeVertices(point vertice[])
{
...
void drawVertices(point vertice[], enum color line_color)
{
	for (size_t i = 0; i < VERTICES; i++) {
		switch (i) {
		case VERTICES - 1:
			gfx_line((int)vertice[i].x, (int)vertice[i].y, (int)vertice[0].x,
					 (int)vertice[0].y, line_color);
			break;
		default:
			gfx_line((int)vertice[i].x, (int)vertice[i].y,
					 (int)vertice[i + 1].x, (int)vertice[i + 1].y, line_color);
			break;
		}
	}
}

void calculateSideLen(double sides[VERTICES], point vertices[])
{
	for (size_t i = 0; i < VERTICES; i++) {
		switch (i) {
		case VERTICES - 1:
			sides[i] = hypot((vertices[i].x - vertices[0].x),
							 (vertices[i].y - vertices[0].y));
			break;
		default:
			sides[i] = hypot((vertices[i].x - vertices[i + 1].x),
...
double calculateAndPrintSideError(point vertices[])
{
	double sides[VERTICES];
	calculateSideLen(sides, vertices);
	double errors[VERTICES];
	double mean_error = 0;
	for (size_t i = 0; i < VERTICES; i++) {
		switch (i) {
		case VERTICES - 1:
			errors[i] = pow(((sides[0] - sides[i]) / (sides[0] + sides[i])), 2);
			break;
		default:
			errors[i] =
				pow(((sides[i + 1] - sides[i]) / (sides[i + 1] + sides[i])), 2);
			break;
		}
		mean_error += errors[i];
	}
